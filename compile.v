...
Fixpoint compile (c : cpGCL) (st : St) : tree St :=
  match c with
  | CSkip => Leaf st
  | CAbort => Fix st (const true) (@Leaf _) (@Leaf _)
  ...
  | CUniform e k => tree_bind (uniform_tree (as_nat (e st)))
                     (fun n => compile (k n) st)
  | CWhile e body => Fix st e (compile body) (@Leaf _)
  | CObserve e => if e st then Leaf st else (@Fail _)
  end.
...
Proof.
  ...
  - destruct (e st); auto.
  - constructor; auto.
  - constructor; intros []; simpl; auto; apply wf_tree_btree_to_tree.
...
Lemma compile_uniform e k (st : St) :
  compile (CUniform e k) st =
    tree_bind (uniform_tree (as_nat (e st))) (fun n => compile (k n) st).
