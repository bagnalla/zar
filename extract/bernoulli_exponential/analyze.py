...
gamma = 10
def pmf(b):
    return exp(-gamma) if b == 1 else (1 - exp(-gamma) if b == 0 else 0)
...
def smape(d):
    return sum(abs(p - pmf(i)) / (abs(p) + pmf(i)) for i, p in d) \
        / len(d)
...
def analyze(records):
    ...
    # true_d = list(takewhile(lambda x: x[1] > 0, ((i, 0.5) for i in count(3))))
    true_d = [(0, pmf(0)), (1, pmf(1))]
