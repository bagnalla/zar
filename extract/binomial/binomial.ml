...
(** val binomial : q -> nat -> cpGCL **)

let binomial p n =
  CSeq ((CAssign (('i'::[]), (const_val (Vnat O)))), (CSeq ((CAssign (('k'::[]),
    (const_val (Vnat O)))), (CWhile ((fun s -> Nat.ltb (as_nat (s ('i'::[]))) n), (CSeq ((CChoice
    ((const p), (fun b -> if b then incr ('k'::[]) else CSkip))), (incr ('i'::[])))))))))

(** val sampler : (__, nat) itree **)

let sampler =
  ITree.map (fun s -> as_nat (s ('n'::[])))
    (cpGCL_to_itree
      (binomial { qnum = (Zpos XH); qden = (XO XH) } (S (S (S (S (S (S (S (S O))))))))) empty)
