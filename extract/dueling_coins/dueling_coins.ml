...
(** val dueling_coins : q -> cpGCL **)

let dueling_coins p =
  CSeq ((CAssign (('a'::[]), (const_val (Vbool false)))), (CSeq ((CAssign (('b'::[]),
    (const_val (Vbool false)))), (CWhile ((fun s ->
    eqb (as_bool (s ('a'::[]))) (as_bool (s ('b'::[])))), (CSeq ((flip ('a'::[]) p),
    (flip ('b'::[]) p))))))))

(** val sampler : (__, bool) itree **)

let sampler =
  ITree.map (fun s -> as_bool (s ('a'::[])))
    (cpGCL_to_itree (dueling_coins { qnum = (Zpos XH); qden = (XO (XO (XI (XO XH)))) }) empty)
