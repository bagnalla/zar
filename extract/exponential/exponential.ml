 ...
 struct
  ...
  let rec pos_div_eucl a b =
    ...
    | XI a' ->
      ...
      if ltb r' b
      then Pair ((mul (Zpos (XO XH)) q0), r')
      else Pair ((add (mul (Zpos (XO XH)) q0) (Zpos XH)), (sub r' b))
    | XO a' ->
      let Pair (q0, r) = pos_div_eucl a' b in
    ...
    | Zpos a0 ->
      (match b with
       ...
       | Zpos b0 ->
         ...
         let Pair (aa, bb) = p in Pair ((Zpos g), (Pair ((Zpos aa), (Zpos bb))))
       | Zneg b0 ->
         let Pair (g, p) = Coq_Pos.ggcd a0 b0 in
         let Pair (aa, bb) = p in Pair ((Zpos g), (Pair ((Zpos aa), (Zneg bb)))))
    | Zneg a0 ->
      (match b with
       | Z0 -> Pair ((abs a), (Pair ((sgn a), Z0)))
       | Zpos b0 ->
         let Pair (g, p) = Coq_Pos.ggcd a0 b0 in
         let Pair (aa, bb) = p in Pair ((Zpos g), (Pair ((Zneg aa), (Zpos bb))))
       | Zneg b0 ->
         let Pair (g, p) = Coq_Pos.ggcd a0 b0 in
         let Pair (aa, bb) = p in Pair ((Zpos g), (Pair ((Zneg aa), (Zneg bb)))))
 end
...
let qred q0 =
  ...
  let Pair (r1, r2) = snd (Z.ggcd q1 (Zpos q2)) in { qnum = r1; qden = (Z.to_pos r2) }
...
let eqb0 a b =
  ...
  if if if if if if if eqb a0 b0 then eqb a1 b1 else false then eqb a2 b2 else false
              then eqb a3 b3
              else false
           then eqb a4 b4
           else false
...
(** val upd : string -> val0 -> st -> st **)

let upd x v st0 y =
  if eqb1 y x then v else st0 y
...
(** val bernoulli_btree_to_tree : (unit0, bool) sum btree -> tree **)

let bernoulli_btree_to_tree =
  btree_to_tree (cotuple (const (Vint Z0)) (fun x -> Vbool x))

(** val uniform_btree_to_tree : (unit0, nat) sum btree -> tree **)

let uniform_btree_to_tree =
  btree_to_tree (cotuple (const (Vint Z0)) (fun x -> Vnat x))

(** val list_btree_aux : 'a1 list -> nat -> (unit0, 'a1) sum btree **)
...
(** val list_btree : 'a1 list -> (unit0, 'a1) sum btree **)

let list_btree l =
  list_btree_aux l (log2 (next_pow_2 (length l)))

(** val uniform_btree : nat -> (unit0, nat) sum btree **)

let uniform_btree n =
  list_btree (rev_range n)

(** val bernoulli_btree : nat -> nat -> (unit0, bool) sum btree **)
...
let uniform_tree n =
  Fix ((upd _UU03f5_ (Vint Z0) empty), (fun s -> negb (is_nat (s _UU03f5_))), (fun _ ->
    uniform_tree_open n), (fun x -> Leaf x))
...
let rec compile c st0 =
  ...
  | CIte (e, c1, c2) -> if as_bool (e st0) then compile c1 st0 else compile c2 st0
  ...
  | CWhile (e, body) -> Fix (st0, (compose as_bool e), (compile body), (fun x -> Leaf x))
  | CObserve e -> if as_bool (e st0) then Leaf st0 else Fail
...
(** val to_itree : tree -> (__, (unit0, st) sum) itree **)

let rec to_itree = function
| Leaf x -> ret (Obj.magic monad_itree) (Inr x)
| Fail -> ret (Obj.magic monad_itree) (Inl Tt)
| Choice (_, k) -> lazy (Go (VisF (__, (compose to_itree (Obj.magic k)))))
| Fix (st0, g, g0, k) ->
  ITree.iter (fun s ->
    if g s
    then ITree.bind (to_itree (g0 s)) (fun y ->
           match y with
           | Inl _ -> ret (Obj.magic monad_itree) (Inr (Inl Tt))
           | Inr s' -> ret (Obj.magic monad_itree) (Inl s'))
    else ITree.map (fun x -> Inr x) (to_itree (k s))) st0

(** val tie_itree : ('a2, (unit0, 'a1) sum) itree -> ('a2, 'a1) itree **)
...
let incr v = match v with
| Vbool _ -> v
| Vnat n -> Vnat (S n)
| Vint n -> Vint (Z.add n (Zpos XH))

(** val is_even : val0 -> val0 **)
...
(** val bernoulli_exponential_0_1 : string -> q -> cpGCL **)

let bernoulli_exponential_0_1 out gamma =
  CSeq ((CAssign ((String ((Ascii (true, true, false, true, false, true, true, false)),
    EmptyString)), (const_val (Vnat O)))), (CSeq ((CAssign ((String ((Ascii (true, false, false,
    false, false, true, true, false)), EmptyString)), (const_val (Vbool true)))), (CSeq ((CWhile
    ((fun s ->
    s (String ((Ascii (true, false, false, false, false, true, true, false)), EmptyString))),
    (CChoice ((fun s ->
    qdiv gamma
      (n2Q (S
        (as_nat
          (s (String ((Ascii (true, true, false, true, false, true, true, false)), EmptyString))))))),
    (fun b ->
    if b
    then CAssign ((String ((Ascii (true, true, false, true, false, true, true, false)),
           EmptyString)), (fun s ->
           incr
             (s (String ((Ascii (true, true, false, true, false, true, true, false)),
               EmptyString)))))
    else CAssign ((String ((Ascii (true, false, false, false, false, true, true, false)),
           EmptyString)), (const_val (Vbool false)))))))), (CIte ((fun s ->
    is_even
      (s (String ((Ascii (true, true, false, true, false, true, true, false)), EmptyString)))),
    (CAssign (out, (const_val (Vbool true)))), (CAssign (out, (const_val (Vbool false)))))))))))

(** val bernoulli_exponential : string -> q -> cpGCL **)

let bernoulli_exponential out gamma =
  if qle_bool gamma { qnum = (Zpos XH); qden = XH }
  then bernoulli_exponential_0_1 out gamma
  else CSeq ((CAssign ((String ((Ascii (true, false, false, true, false, true, true, false)),
         EmptyString)), (const_val (Vnat (S O))))), (CSeq ((CAssign ((String ((Ascii (false,
         true, false, false, false, true, true, false)), EmptyString)),
         (const_val (Vbool true)))), (CSeq ((CWhile ((fun s -> Vbool
         (if as_bool
               (s (String ((Ascii (false, true, false, false, false, true, true, false)),
                 EmptyString)))
          then qle_bool
                 (n2Q
                   (as_nat
                     (s (String ((Ascii (true, false, false, true, false, true, true, false)),
                       EmptyString))))) gamma
          else false)), (CSeq
         ((bernoulli_exponential_0_1 (String ((Ascii (false, true, false, false, false, true,
            true, false)), EmptyString)) { qnum = (Zpos XH); qden = XH }), (CAssign ((String
         ((Ascii (true, false, false, true, false, true, true, false)), EmptyString)), (fun s ->
         incr
           (s (String ((Ascii (true, false, false, true, false, true, true, false)),
             EmptyString)))))))))), (CIte ((fun s ->
         s (String ((Ascii (false, true, false, false, false, true, true, false)), EmptyString))),
         (bernoulli_exponential_0_1 out (qminus gamma { qnum = (qfloor gamma); qden = XH })),
         (CAssign (out, (const_val (Vnat O)))))))))))

(** val sampler : (__, bool) itree **)

let sampler =
  ITree.map (fun s -> as_bool (s EmptyString))
    (cpGCL_to_itree
      (bernoulli_exponential EmptyString { qnum = (Zpos (XI (XO (XO XH)))); qden = (XO (XI (XO
        XH))) }) empty)
