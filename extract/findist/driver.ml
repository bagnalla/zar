...
let () =
  ...
  let sampler = findist_itree @@ List.map nat_of_int weights in
  (* let t1 = Sys.time() in *)
  (* print_endline (string_of_float (t1 -. t0)); *)

  let sum = List.fold_left (+.) 0.0 @@ List.map float_of_int weights in
  print_endline ("expected mu: " ^
                   string_of_float @@
                     List.fold_left (+.) 0.0
                       (List.mapi (fun i n -> float_of_int i *.
                                                (float_of_int n /. sum)) weights));
