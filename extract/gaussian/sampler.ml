...
let bernoulli_tree p =
  Fix ((upd _UU03f5_ (Vint Z0) empty), (fun s -> is_int (s _UU03f5_)),
    (fun _ -> bernoulli_tree_open (Z.to_nat p.qnum) (Coq_Pos.to_nat p.qden)),
    (fun x -> Leaf x))
...
let uniform_tree n =
  ...
  Fix ((upd _UU03f5_ (Vint Z0) empty), (fun s -> negb (is_nat (s _UU03f5_))),
  (fun _ -> t), (fun x -> Leaf x))
...
| Choice (p, f) ->
  ...
  else tree_bind (bernoulli_tree p) (fun s ->
         debias (f (as_bool (s _UU03f5_))))
...
| Fix (st0, g, g0, k) ->
  Fix (st0, g, (compose elim_choices g0), (compose elim_choices k))
| x -> x
...
let cpGCL_to_itree c =
  compose (compose (compose (compose to_itree opt) debias) elim_choices)
    (compile c)

(** val flip : char list -> q -> cpGCL **)

let flip x p =
  CChoice ((const p), (fun b -> CAssign (x, (const_val (Vbool b)))))
...
let bernoulli_exponential_0_1 out gamma =
  CSeq ((CAssign (('k'::[]), (const_val (Vnat O)))), (CSeq ((CAssign
    (('a'::[]), (const_val (Vbool true)))), (CSeq ((CWhile ((fun s ->
    as_bool (s ('a'::[]))), (CChoice ((fun s ->
    qdiv (gamma s) (n2Q (S (as_nat (s ('k'::[])))))), (fun b ->
    if b
    then CAssign (('k'::[]), (fun s -> succ0 (s ('k'::[]))))
    else CAssign (('a'::[]), (const_val (Vbool false)))))))), (CIte
    ((fun s -> is_even (s ('k'::[]))), (CAssign (out,
    (const_val (Vbool true)))), (CAssign (out,
...
let bernoulli_exponential out gamma =
  CIte ((fun s -> qle_bool (gamma s) { qnum = (Zpos XH); qden = XH }),
    ...
    (const_val (Vnat (S O))))), (CSeq ((CAssign (('b'::[]),
    (const_val (Vbool true)))), (CSeq ((CWhile ((fun s ->
    (&&) (as_bool (s ('b'::[])))
      (qle_bool (n2Q (as_nat (s ('i'::[])))) (gamma s))), (CSeq
    ((bernoulli_exponential_0_1 ('b'::[])
       (const { qnum = (Zpos XH); qden = XH })), (CAssign (('i'::[]),
    (fun s -> succ0 (s ('i'::[]))))))))), (CIte ((fun s ->
    as_bool (s ('b'::[]))),
    (bernoulli_exponential_0_1 out (fun s ->
      qminus (gamma s) { qnum = (qfloor (gamma s)); qden = XH })), (CAssign
    (out, (const_val (Vnat O)))))))))))))

(** val laplace : char list -> nat -> nat -> cpGCL **)

let laplace out s t =
  CSeq ((CAssign (('l'::('p'::[])), (const_val (Vbool true)))), (CWhile
    ((fun s0 -> as_bool (s0 ('l'::('p'::[])))), (CUniform ((const t),
    (fun u -> CSeq
    ((bernoulli_exponential ('d'::[]) (fun _ -> { qnum =
       (Z.of_nat (min u t)); qden = (Coq_Pos.of_nat t) })), (CIte ((fun s0 ->
    as_bool (s0 ('d'::[]))), (CSeq ((CAssign (('v'::[]), (fun _ -> Vnat O))),
    (CSeq
    ((bernoulli_exponential ('i'::('l'::[]))
       (const { qnum = (Zpos XH); qden = XH })), (CSeq ((CWhile ((fun s0 ->
    as_bool (s0 ('i'::('l'::[])))), (CSeq ((CAssign (('v'::[]), (fun s0 ->
    Vnat (S (as_nat (s0 ('v'::[]))))))),
    (bernoulli_exponential ('i'::('l'::[]))
      (const { qnum = (Zpos XH); qden = XH })))))), (CSeq ((CAssign
    (('x'::[]), (fun s0 -> Vnat (add u (mul t (as_nat (s0 ('v'::[])))))))),
    (CSeq ((CAssign (('y'::[]), (fun st0 -> Vnat
    (Nat.div (as_nat (st0 ('x'::[]))) s)))), (CSeq
    ((flip ('c'::[]) { qnum = (Zpos XH); qden = (XO XH) }), (CIte ((fun s0 ->
    (&&) (as_bool (s0 ('c'::[]))) (Nat.eqb (as_nat (s0 ('y'::[]))) O)),
    CSkip, (CSeq ((CAssign (('l'::('p'::[])), (fun _ -> Vbool false))),
    (CAssign (out, (fun s0 -> Vint
    (Z.mul
      (Z.sub (Zpos XH)
        (Z.mul (Zpos (XO XH))
          (if as_bool (s0 ('c'::[])) then Zpos XH else Z0)))
      (Z.of_nat (as_nat (s0 ('y'::[])))))))))))))))))))))))), CSkip)))))))))

(** val gaussian : char list -> q -> cpGCL **)

let gaussian out sigma =
  let t = S (Z.to_nat (qfloor sigma)) in
  CSeq ((CAssign (('o'::('l'::[])), (const_val (Vbool false)))), (CSeq
  ((CWhile ((fun s -> negb (as_bool (s ('o'::('l'::[]))))), (CSeq
  ((laplace ('z'::[]) (S O) t),
  (bernoulli_exponential ('o'::('l'::[])) (fun s ->
    let q0 =
      qdiv
        (qpower
          (qminus (qabs { qnum = (as_int (s ('z'::[]))); qden = XH })
            (qdiv (qpower sigma (Zpos (XO XH))) (n2Q t))) (Zpos (XO XH)))
        (qmult { qnum = (Zpos (XO XH)); qden = XH }
          (qpower sigma (Zpos (XO XH))))
    in
    if qle_bool { qnum = Z0; qden = XH } q0
    then q0
    else { qnum = Z0; qden = XH })))))), (CAssign (out, (fun s ->
  s ('z'::[])))))))

(** val gauss : char list -> z -> q -> cpGCL **)

let gauss out mu sigma =
  CSeq ((gaussian ('x'::('1'::('2'::('3'::[])))) sigma), (CAssign (out,
    (fun s -> Vint (Z.add (as_int (s ('x'::('1'::('2'::('3'::[])))))) mu)))))

(** val sampler : z -> q -> (__, z) itree **)

let sampler mu sigma =
  ITree.map (fun s -> as_int (s []))
    (cpGCL_to_itree (gauss [] mu sigma) empty)
