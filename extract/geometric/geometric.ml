...
| Choice (p, f) ->
  ...
  else tree_bind (bernoulli_tree p) (fun s -> debias (f (as_bool (s _UU03f5_))))
...
(** val incr : char list -> cpGCL **)

let incr x =
  CAssign (x, (fun s -> Vnat (S (as_nat (s x)))))
...
(** val divisible_by : nat -> nat -> bool **)

let divisible_by n m =
  Nat.eqb (Nat.modulo n m) O
  ...
  (match n0 with
   ...
   | S n1 ->
     (match n1 with
      | O -> true
      | S _ ->
        (&&) (negb (divisible_by n (S (S O))))
          (fold_left (fun acc m -> (&&) acc (negb (divisible_by n m)))
            (drop (S (S (S O))) (range (S (Nat.sqrt n)))) true)))

(** val geometric : q -> cpGCL **)

let geometric p =
  CSeq ((CAssign (('h'::[]), (const_val (Vnat O)))), (CSeq ((flip ('x'::[]) p), (CSeq ((CWhile
    ((fun s -> as_bool (s ('x'::[]))), (CSeq ((incr ('h'::[])), (flip ('x'::[]) p))))), (CObserve
    (fun s -> is_prime (as_nat (s ('h'::[]))))))))))

(** val sampler : (__, nat) itree **)

let sampler =
  ITree.map (fun s -> as_nat (s ('h'::[])))
    (cpGCL_to_itree (geometric { qnum = (Zpos (XO XH)); qden = (XI XH) }) empty)
