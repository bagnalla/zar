...
let rec to_itree_ = function
| Leaf x -> ret (Obj.magic monad_itree) (Inr x)
| Fail -> ret (Obj.magic monad_itree) (Inl ())
| Choice (_, k) -> lazy (Go (VisF (__, (compose to_itree_ (Obj.magic k)))))
| Fix (st0, g, g0, k) ->
  ITree.iter (fun s ->
    if g s
    then ITree.bind (to_itree_ (g0 s)) (fun y ->
           match y with
           | Inl _ -> ret (Obj.magic monad_itree) (Inr (Inl ()))
           | Inr s' -> ret (Obj.magic monad_itree) (Inl s'))
    else ITree.map (fun x -> Inr x) (to_itree_ (k s))) st0
...
let gp =
  CSeq ((flip ('f'::[]) { qnum = (Zpos XH); qden = (XO XH) }), (CSeq ((CChoice
    ((const { qnum = (Zpos XH); qden = (XO XH) }), (fun b ->
    if b
    then CAssign (('r'::[]), (fun s -> s ('f'::[])))
    else CAssign (('r'::[]), (fun x -> Vbool (const true x)))))), (CObserve (fun s ->
    s ('r'::[]))))))
