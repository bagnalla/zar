...
let laplace out s t =
  CSeq ((CAssign (('l'::('p'::[])), (const_val (Vbool true)))), (CWhile ((fun s0 ->
    as_bool (s0 ('l'::('p'::[])))), (CUniform ((const t), (fun u -> CSeq
    ((bernoulli_exponential ('d'::[]) (fun _ -> { qnum = (Z.of_nat (min u t)); qden =
       (Coq_Pos.of_nat t) })), (CIte ((fun s0 -> as_bool (s0 ('d'::[]))), (CSeq ((CAssign
    (('v'::[]), (fun _ -> Vnat O))), (CSeq
    ((bernoulli_exponential ('i'::('l'::[])) (const { qnum = (Zpos XH); qden = XH })), (CSeq
    ((CWhile ((fun s0 -> as_bool (s0 ('i'::('l'::[])))), (CSeq ((CAssign (('v'::[]), (fun s0 ->
    Vnat (S (as_nat (s0 ('v'::[]))))))),
    (bernoulli_exponential ('i'::('l'::[])) (const { qnum = (Zpos XH); qden = XH })))))), (CSeq
    ((CAssign (('x'::[]), (fun s0 -> Vnat (add u (mul t (as_nat (s0 ('v'::[])))))))), (CSeq
    ((CAssign (('y'::[]), (fun st0 -> Vnat (Nat.div (as_nat (st0 ('x'::[]))) s)))), (CSeq
    ((flip ('c'::[]) { qnum = (Zpos XH); qden = (XO XH) }), (CIte ((fun s0 ->
    (&&) (as_bool (s0 ('c'::[]))) (Nat.eqb (as_nat (s0 ('y'::[]))) O)), CSkip, (CSeq ((CAssign
    (('l'::('p'::[])), (fun _ -> Vbool false))), (CAssign (out, (fun s0 -> Vint
    (Z.mul
      (Z.sub (Zpos XH) (Z.mul (Zpos (XO XH)) (if as_bool (s0 ('c'::[])) then Zpos XH else Z0)))
...
let gaussian out sigma =
  ...
  CSeq ((CAssign (('o'::('l'::[])), (const_val (Vbool false)))), (CSeq ((CWhile ((fun s ->
  negb (as_bool (s ('o'::('l'::[]))))), (CSeq ((laplace ('z'::[]) (S O) t),
  (bernoulli_exponential ('o'::('l'::[])) (fun s ->
    let q0 =
      qdiv
        ...
        (qmult { qnum = (Zpos (XO XH)); qden = XH } (qpower sigma (Zpos (XO XH))))
    in
    if qle_bool { qnum = Z0; qden = XH } q0 then q0 else { qnum = Z0; qden = XH })))))), (CAssign
  (out, (fun s -> s ('z'::[])))))))

(** val gauss' : char list -> (st -> z) -> q -> cpGCL **)

let gauss' x mu sigma =
  CSeq ((gaussian x sigma), (CAssign (x, (fun s -> Vint (Z.add (as_int (s x)) (mu s))))))

(** val unif : char list -> (st -> nat) -> cpGCL **)

let unif x e =
  CUniform (e, (fun n -> CAssign (x, (const_val (Vint (Z.of_nat n))))))

(** val hare_tortoise : cpGCL **)

let hare_tortoise =
  CSeq ((unif ('t'::('0'::[])) (const (S (S (S (S (S (S (S (S (S (S O)))))))))))), (CSeq
    ((CAssign (('t'::[]), (fun s -> s ('t'::('0'::[]))))), (CSeq ((CAssign (('h'::[]),
    (const_val (Vint Z0)))), (CSeq ((CAssign (('t'::('i'::('m'::('e'::[])))),
    (const_val (Vint Z0)))), (CSeq ((CWhile ((fun s ->
    Z.ltb (as_int (s ('h'::[]))) (as_int (s ('t'::[])))), (CSeq ((CAssign (('t'::[]), (fun s ->
    Vint (Z.add (as_int (s ('t'::[]))) (Zpos XH))))), (CSeq ((CChoice
    ((const { qnum = (Zpos (XO XH)); qden = (XI (XO XH)) }), (fun b ->
    if b
    then CSeq
           ((gauss' ('g'::[]) (const (Zpos (XO (XO XH)))) { qnum = (Zpos (XO XH)); qden = XH }),
           (CAssign (('h'::[]), (fun s -> Vint
           (Z.add (as_int (s ('h'::[]))) (as_int (s ('g'::[]))))))))
    else CSkip))), (CAssign (('t'::('i'::('m'::('e'::[])))), (fun s -> Vint
    (Z.add (as_int (s ('t'::('i'::('m'::('e'::[])))))) (Zpos XH))))))))))), (CObserve (fun s ->
    Z.leb (Zpos (XO (XO (XI (XO XH))))) (as_int (s ('t'::('i'::('m'::('e'::[])))))))))))))))))

(** val sampler : (__, z) itree **)

let sampler =
  ITree.map (fun s -> as_int (s ('t'::('0'::[])))) (cpGCL_to_itree hare_tortoise empty)
