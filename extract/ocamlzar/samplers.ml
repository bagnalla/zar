...
let observe t =
  let Go _observe = Lazy.force t in _observe
...
(** val to_itree_open : 'a1 tree -> (__ boolE, (unit, 'a1) sum) itree **)

let rec to_itree_open = function
| Leaf x -> ret (Obj.magic monad_itree) (Inr x)
| Fail -> ret (Obj.magic monad_itree) (Inl ())
| Choice (_, k) ->
  lazy (Go (VisF (GetBool, (compose to_itree_open (Obj.magic k)))))
| Fix (st, g, g0, k) ->
  ITree.iter (fun s ->
    if g s
...
(** val to_itree : 'a1 tree -> (__ boolE, 'a1) itree **)
...
(** val findist_itree : z list -> (__ boolE, z) itree **)

let findist_itree weights =
  to_itree (findist_tree weights)

type samplerPackage = { coin_sampler : (q -> (__ boolE, bool) itree);
                        die_sampler : (z -> (__ boolE, z) itree);
                        findist_sampler : (z list -> (__ boolE, z) itree) }

(** val coin_itree : q -> (__ boolE, bool) itree **)

let coin_itree p =
  to_itree (bernoulli_tree (qred p))

(** val die_itree : z -> (__ boolE, z) itree **)
