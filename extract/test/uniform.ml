...
let rec of_uint_acc d acc =
  ...
  | D4 d0 ->
    of_uint_acc d0 (S (S (S (S (tail_mul (S (S (S (S (S (S (S (S (S (S O)))))))))) acc)))))
  | D5 d0 ->
    of_uint_acc d0 (S (S (S (S (S (tail_mul (S (S (S (S (S (S (S (S (S (S O)))))))))) acc))))))
  | D6 d0 ->
...
let rec of_hex_uint_acc d acc =
  ...
  | D10 d0 ->
    of_hex_uint_acc d0
      (tail_mul (S (S (S (S (S (S (S (S (S (S (S (S (S (S (S (S O)))))))))))))))) acc)
  | D11 d0 ->
...
(** val to_itree : tree -> (__, (unit, st) sum) itree **)
...
(** val uniform : char list -> nat -> cpGCL **)

let uniform x n =
  CUniform ((const n), (fun i -> CAssign (x, (fun x0 -> Vnat (const i x0)))))

(** val sampler : (__, nat) itree **)

let sampler =
  ITree.map (fun s -> as_nat (s []))
    (cpGCL_to_itree (uniform [] (of_num_uint (UIntDecimal (D1 (D0 (D0 (D0 (D0 Nil)))))))) empty)
